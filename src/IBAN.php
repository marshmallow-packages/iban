...
class IBAN
{
    public static function validate($iban)
    {
        ...
        if (isset($countries[substr($iban,0,2)])) {
            if(strlen($iban) == $countries[substr($iban,0,2)]){
                ...
                $moved_char_arr = str_split($moved_char);
                $new_string = "";
    ...
    public static function getBankAccount ($iban)
    {
        if (self::validate($iban)) {
            return ltrim(substr($iban, -10, 10), '0');
        }
        return null;
    }
    ...
    {
        if (self::validate($iban)) {
            $bic_list = require __DIR__ . '/bic-list.php';
            $bic_identifier = substr($iban, 4, 4);
            foreach ($bic_list as $key => $bank) {
                if ($bank['identifier'] == $bic_identifier) {
                    $bank['iban'] = $iban;
                    $bank['account_number'] = self::getBankAccount($iban);
                    return $bank;
                }
            }
        }
